...
- large depth shadow contamination | emboss outer shadow | `updateStyleDepth` clamps `depth.abs()` by widget radius, but the clamped depth still drives both path offset and `MaskFilter.blur`; emboss paints light and dark external shadows in two independent `saveLayer` passes and only subtracts the original shape, so the two restored shadows can overlap on the final canvas and visually mix into gray or darkened residue | code-confirmed with rendering inference | high
- large depth inner-shadow overlap | deboss inner shadow | deboss `_paintShadows` also composes independent light and dark passes inside the original path using scaled `dstOut` masks; when main `depth` is large, the surviving inner highlight and inner shadow bands can overlap inside the shape because there is no pass that subtracts the opposite shadow region | code-confirmed with rendering inference | medium
- subpixel border depth direction loss | emboss neumorphic border | when `borderDepth` is close to `0.1`, `borderScaleX/Y` stay near `1`, border mask translations are subpixel-sized, and the enlarged/transformed mask is almost coincident with the original path; the intended light-source-directed top-left highlight and bottom-right shade can degrade into light and dark anti-aliased residue around most or all of the outline | code-confirmed with rendering inference | high
- subpixel border depth lip collapse | deboss neumorphic border | deboss `_paintBorder` translates blurred light/dark paths by `borderDepthOffset` and subtracts the original path; when `borderDepthOffset` and `borderMaskFilter` are both subpixel-sized, the inset lip can collapse into a thin outline residue with weak directionality, so light and dark colors may appear around the whole opening | code-confirmed with rendering inference | medium
- explicit border fallback limitation | direct stroke border | `NeumorphicBorder.isEnabled=true` draws a single-color stroke with `border.color` and `border.width`; it cannot preserve the light-source-dependent two-color highlight/shade behavior by itself, so a small-`borderDepth` fallback would need a directional gradient stroke or clipped dual-stroke approach | code-confirmed | high
...
- shadow and border rendering defects | large `depth` and subpixel `borderDepth` failure modes are confirmed from painter pass structure, mask geometry, blur, and translation formulas, but exact visual thresholds depend on device pixel ratio, Flutter renderer backend, shape complexity, and color contrast | screenshot or golden rendering tests
